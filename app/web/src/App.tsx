...
import { TaskAtlas } from "./components/TaskAtlas";
...
type BoardView = "issues" | "workflow" | "atlas";
const SHOW_WORKFLOW_BOARD_ENTRY = false;
const HIDDEN_LABELS_KEY = "taskboard.hidden-labels";
...
function readHiddenLabels(): Record<string, string[]> {
  try {
    const value = JSON.parse(window.localStorage.getItem(HIDDEN_LABELS_KEY) ?? "{}");
    if (!value || typeof value !== "object" || Array.isArray(value)) return {};
    return Object.fromEntries(Object.entries(value).filter((entry): entry is [string, string[]] => (
      Array.isArray(entry[1]) && entry[1].every((label) => typeof label === "string")
    )));
  } catch {
    return {};
  }
...
export function App() {
  ...
  const [hiddenLabels, setHiddenLabels] = useState(readHiddenLabels);
  ...
  const [boardView, setBoardView] = useState<BoardView>("atlas");
  ...
  const availableLabels = useMemo(
    () => [...new Set([
      ...DEFAULT_LABELS.map((label) => label.name).filter((label) => !hiddenLabels[selectedProjectId]?.includes(label)),
      ...tasks.flatMap((task) => task.labels),
    ])].filter((label) => label !== "想法池" && !label.startsWith("atlas-order:")),
    [hiddenLabels, selectedProjectId, tasks],
  );
  ...
  async function createAtlasIdea(title: string, description: string) {
    if (!selectedProjectId) throw new Error("请先选择项目");
    const saved = await createTaskRequest(selectedProjectId, {
      title, description, status: "backlog", priority: "none", labels: ["想法池"],
      assigneeTarget: "current-user", workflowId: null, developmentContext: null,
    ...
    setProjects(current => current.map(project => project.id === selectedProjectId
      ? {...project, issueCount: project.issueCount + 1} : project));
    return saved;
  }

  async function promoteAtlasIdea(idea: Task, parent: Task | null, order: number) {
    let linked = idea;
    if (parent && idea.relations.parent?.id !== parent.id) {
      const result = await addTaskRelation(idea, "parent", parent.id);
      linked = result.task;
      setTasks(current => sortTasks(current.map(task =>
        task.id === result.task.id ? result.task : task.id === result.relatedTask.id ? result.relatedTask : task)));
    } else if (!parent && idea.relations.parent) {
      const result = await removeTaskRelation(idea, "parent", idea.relations.parent.id);
      linked = result.task;
      setTasks(current => sortTasks(current.map(task =>
        task.id === result.task.id ? result.task : task.id === result.relatedTask.id ? result.relatedTask : task)));
    }
    try {
      const promoted = await updateTaskRequest(linked, {
        ...taskToDraft(linked),
        labels: [...linked.labels.filter(label => label !== "想法池" && !label.startsWith("atlas-order:")), ...(parent ? [`atlas-order:${order}`] : [])],
      });
      setTasks(current => sortTasks(current.map(task => task.id === promoted.id ? promoted : task)));
      return promoted;
    } catch (error) {
      if (selectedProjectId) void refreshTasks(selectedProjectId, { quiet: true });
      throw error;
  ...
  async function deleteProjectLabel(label: string) {
    if (!selectedProjectId || label === "想法池" || label.startsWith("atlas-order:")) return;
    const projectId = selectedProjectId;
    try {
      const projectTasks = await listTasks(projectId, undefined, true);
      for (const task of projectTasks.filter((candidate) => candidate.labels.includes(label))) {
      ...
      setHiddenLabels((current) => {
        const next = { ...current, [projectId]: [...new Set([...(current[projectId] ?? []), label])] };
        window.localStorage.setItem(HIDDEN_LABELS_KEY, JSON.stringify(next));
        return next;
      });
      await refreshTasks(projectId, { quiet: true });
    } catch (error) {
      await refreshTasks(projectId, { quiet: true });
      throw error;
    }
  ...
  return (
    <div className={`app-shell${embedded ? " embedded" : ""}`} style={appShellStyle}>
      <VersionNotice />
      <a className="board-guide-link" href={`/guide.html${window.location.search}`}>使用指南</a>
      ...
      <main className="workspace">
        ...
        {selectedProjectId && !detailTask && <div className="board-toolbar">
          <div className="view-tabs" aria-label="看板视图">
            <button className={`view-tab${boardView === "atlas" ? " active" : ""}`} aria-pressed={boardView === "atlas"} onClick={() => selectBoardView("atlas")}>任务图谱</button>
        ...
        {!selectedProjectId ? (
          ...
          <TaskDetail
            ...
            onDeleteLabel={deleteProjectLabel}
          ...
          <TaskAtlas key={selectedProjectId} projectId={selectedProjectId} tasks={tasks} onOpen={openTaskDetail} onCreateIdea={createAtlasIdea} onPromoteIdea={promoteAtlasIdea} onStatus={(task,status)=>updateTaskProperties(task,{status})}/>
        ) : boardView === "workflow" ? (
