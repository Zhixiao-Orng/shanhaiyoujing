...
export function TaskDetail({
  ...
  const propertyTitleRef = useRef<HTMLInputElement>(null);
  ...
  useEffect(() => {
    setCurrentTask(task);
    if (document.activeElement !== titleRef.current && document.activeElement !== propertyTitleRef.current) setTitle(task.title);
  ...
  async function submitWorkLog() {
    const entry = workLogDraft.trim();
    if (!entry || savingWorkLog) return;
    setSavingWorkLog(true);
    setWorkLogError(null);
    try {
      const comment = await createComment(task.id, `${WORK_LOG_PREFIX}${entry}`);
      setComments((current) => [...current, comment]);
      setWorkLogDraft("");
      onAnnounce("工作日志已保存。");
    } catch (error) {
      setWorkLogError(messageFor(error));
    } finally {
        ...
        <div className="issue-detail-layout">
          <div className="issue-detail-main">
            ...
            <section className="activity-section" aria-labelledby="activity-heading">
              <header className="activity-heading">
                ...
                <span>{comments.filter((comment) => !comment.body.startsWith(WORK_LOG_PREFIX)).length}</span>
              </header>
              ...
              <div className="activity-stream">
                ...
                ) : comments.filter((comment) => !comment.body.startsWith(WORK_LOG_PREFIX)).map((comment) => (
                  <article
          ...
          <aside className="issue-properties" aria-label="议题属性">
            ...
            <label className="detail-property-row">
              <span className="detail-property-icon" aria-hidden="true"><LinearIcon name="write" /></span>
              <span className="detail-property-label">标题</span>
              <input
                ref={propertyTitleRef}
                type="text"
                aria-label="任务标题"
                value={title}
                disabled={savingProperty === "title"}
            ...
            <section className="detail-work-log" aria-label="工作日志">
              <h2>工作日志</h2>
              <form onSubmit={(event) => { event.preventDefault(); void submitWorkLog(); }}>
                <textarea
                  aria-label="记录工作推进和注意事项"
                  placeholder="记录工作推进、注意事项…"
                  rows={3}
                  value={workLogDraft}
                  onChange={(event) => setWorkLogDraft(event.target.value)}
                />
                <button type="submit" disabled={!workLogDraft.trim() || savingWorkLog}>
                  {savingWorkLog ? "保存中…" : "添加记录"}
                </button>
              </form>
              {workLogError && <p role="alert">{workLogError}</p>}
              {commentsLoading ? <p>正在加载…</p> : (
                <div className="detail-work-log-entries">
                  {comments.filter((comment) => comment.body.startsWith(WORK_LOG_PREFIX)).length === 0 && <p>还没有记录</p>}
                  {comments.filter((comment) => comment.body.startsWith(WORK_LOG_PREFIX)).slice().reverse().map((comment) => (
                    <article key={comment.id}>
                      <time title={exactTime(comment.createdAt)}>{exactTime(comment.createdAt)}</time>
                      <p>{comment.body.slice(WORK_LOG_PREFIX.length)}</p>
                    </article>
                  ))}
                </div>
              )}
