...
function assertTrustedNetworkRequest(request) {
  ...
  if (!isTrustedNetworkHost(originHost) || (new URL(origin).host !== request.headers.host && !process.argv.includes("--dev"))) {
    throw new ApiError(403, "INVALID_ORIGIN", "Request Origin must be local or private");
  }
}
...
export function createTaskboardServer(options = {}) {
  ...
  const server = createServer(async (request, response) => {
    ...
    try {
      ...
      if (pathname.startsWith("/api/local/") || pathname === "/api/device-workspaces"
        || pathname === "/api/workflow-capabilities"
        || /^\/api\/projects\/[^/]+\/(development-contexts|workflow-workspace)$/.test(pathname)) {
        throw new ApiError(404, "NOT_FOUND", "This feature is not available in the local Task Atlas release");
      }
      ...
      if (pathname.startsWith("/api/")) {
        currentCloudConfig = { remoteUrl: null };
